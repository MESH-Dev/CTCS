...
function loadup_scripts() {
    ...
    wp_enqueue_style( 'animate', get_template_directory_uri().'/css/animate.css', '1.0.0', true);
	...
	if (!is_front_page()) {
		...
		if ( is_category() || is_single() ) {
			if (get_post_type() == 'program') {
			    echo "<a href='" . get_post_type_archive_link('program') . "'>Programs</a>";    
			}
			if (get_post_type() == 'jobs') {
			    echo "<a href='" . get_post_type_archive_link('jobs') . "'>Jobs</a>";
			}
			if ( is_single() ) {
				echo $separator;
				the_title();
